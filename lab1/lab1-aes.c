...
void subBytes(int array[4][4])
{
    for(int i=0;i<=3;i++){
        for(int j=0;j<=3;j++){
            array[i][j] = S[getLeft4Bit(array[i][j])][getRight4Bit(array[i][j])];
        }
    }
...
void deSubBytes(int array[4][4])
{
    for(int i=0;i<=3;i++){
        for(int j=0;j<=3;j++){
            array[i][j] = S2[getLeft4Bit(array[i][j])][getRight4Bit(array[i][j])];
        }
    }
...
void mixColumns(int array[4][4])
{
    ...
    for(int i=0;i<=3;i++){
        for(int j=0;j<=3;j++){
            ...
            int tmp_inner[4];
            for(int k=0;k<=3;k++){
                tmp_inner[k] = GFMul(colM[i][k],array[k][j]);
            }
            tmp[i][j] = tmp_inner[0] ^ tmp_inner[1] ^ tmp_inner[2] ^ tmp_inner[3];
        }
    }
    for(int i=0;i<=3;i++){
        for(int j=0;j<=3;j++){
...
void deMixColumns(int array[4][4])
{
    ...
    for(int i=0;i<=3;i++){
        for(int j=0;j<=3;j++){
            ...
            int tmp_inner[4];
            for(int k=0;k<=3;k++){
                tmp_inner[k] = GFMul(deColM[i][k],array[k][j]);
            }
            tmp[i][j] = tmp_inner[0] ^ tmp_inner[1] ^ tmp_inner[2] ^ tmp_inner[3];
        }
    }
...
void addRoundKey(int array[4][4], int round)
{
    ...
    for(int i=0;i<=3;i++){
        int tmp_inner[4];
        splitIntToArray(w[i+round*4],tmp_inner);
        for(int j=0;j<=3;j++){
            tmp[j][i] = tmp_inner[j];
        }
    }
 ...
 */
int RotByte(int num){
    int tmp_inner[4],tmp[4];
    splitIntToArray(num,tmp_inner);
    for(int i=0;i<=3;i++){
        tmp[(i+3)%4] = tmp_inner[i];
    }
    return mergeArrayToInt(tmp);
}
int SubByte(int num){
    int tmp[4],tmp_inner[4];
    splitIntToArray(num,tmp_inner);
    for(int i=0;i<=3;i++){
        tmp[i] = S[getLeft4Bit(tmp_inner[i])][getRight4Bit(tmp_inner[i])];
    }
    return mergeArrayToInt(tmp);
}
void T(int round)
{
    int Nk = 4;
    int temp;
    temp = SubByte(RotByte(w[round-1])) ^ Rcon[(round-4)/4];
    if(round % 4 == 0){
        w[round] = w[round-Nk] ^ temp;
    }
    else{
        w[round] = w[round-Nk] ^ w[round-1];
    }
}
...
void extendKey(char *key)
{
    ...
    for(int i=0;i<=3;i++){
        for(int j=0;j<=3;j++){
            tmp[j] = getIntFromChar(key[i*4+j]);
        }
        w[i] = mergeArrayToInt(tmp);
   }
   for(int i=4;i<=43;i++){
        T(i);
   }
